...
- 활성 기본 모델 버전: `v0.1-real-data`
- 보관된 실험용 버전: `v0.0-pipeline-check`
- 설정 파일: `src/app/config/modelRuntime.ts`
- 활성 프로덕션 모델 경로: `public/models/lightweight_audio_model.production.onnx`
- 보관된 실험용 모델 경로: `public/models/lightweight_audio_model.onnx`

### 현재 모델 상태

활성 브라우저 모델은 다음과 같습니다.
...
- 소스 탐지 동작 확인: guitar F1=0.82, drums AUROC=0.93, vocal AUROC=0.92

전체 per-label 지표, 아티팩트 경로, 알려진 한계는 `ml/model_history.md`를 참고하세요.

### 모델 히스토리

현재까지 3개의 모델 세대가 학습되었습니다.

| 버전 | 상태 | 데이터 | 주요 지표 |
|---|---|---|---|
| `v0.0-pipeline-check` | 보관됨 | Synthetic | 파이프라인 검증용만 |
| `v0.1-real-data` | **활성** | MUSAN + OpenMIC (65K 클립) | Guitar F1=0.82, macro issue F1=0.53 |
| `v0.2-fsd50k-extended` | 학습 중 | + FSD50K partial (85K 클립 예상) | 미정 |

전체 히스토리: `ml/model_history.md`
...
- `MODEL_VERSION === "v0.0-pipeline-check"`이면 보관된 실험용 모델 경로 사용
- `MODEL_VERSION === "v0.1-real-data"` (기본값)이면 프로덕션 모델 경로 사용
- 그 외 비실험용 버전 문자열도 프로덕션 모델 경로로 라우팅됨

### UI 상태 표시

프로덕션 모델이 활성화되어 있을 때 실험용 경고가 표시되지 않습니다. 실험용 경고(`Not production-ready`, `Results may be inaccurate`)는 `MODEL_VERSION === "v0.0-pipeline-check"`일 때만 표시됩니다.
...
브라우저는 `v0.1-real-data` 체크포인트를 제공하며, 호환성 ONNX 계약을 사용합니다.
...
이 모델은 실제 공개 오디오로 학습되어 CI 평가 게이트를 통과했습니다. 실제 오디오에서 의미 있는 소스 및 이슈 예측이 가능합니다.
...
활성 브라우저 모델(`v0.1-real-data`)은 첫 번째 실데이터 체크포인트입니다. 이 모델은 다음을 증명합니다.

- 파이프라인이 실제 오디오에서 end-to-end로 동작함
- 소스 탐지가 실제 녹음에서 일반화됨
- 이슈 탐지가 일반적인 레이블(muddy, harsh, dull)에서 의미 있음

일부 레이블(`boxy`, `nasal`, `thin`)은 학습 데이터 부족으로 약한 성능을 보입니다. FSD50K를 추가한 후속 학습 실행(`v0.2-fsd50k-extended`)이 현재 진행 중입니다.
...
- 활성 브라우저 모델은 `v0.1-real-data` — MUSAN + OpenMIC-2018 (65K 클립) 기반으로 학습됨
- 일부 이슈 레이블(`boxy`, `nasal`, `thin`)은 학습 데이터 부족으로 F1이 낮음
- guitar 임계값이 낮음(0.25) — 데이터 불균형 때문에 비기타 오디오에서 오탐이 발생할 수 있음
- CI golden set이 3개 샘플에 불과 — 임계값이 이 작은 세트에서 과예측을 피하도록 보수적으로 설정됨
- 브라우저 런타임은 아직 Python 모델이 지원하는 full multi-band가 아니라 single-band 호환성 EQ 출력 계약을 사용함
...
- `v0.2-fsd50k-extended` 학습 완료 모니터링 후 baseline 대비 평가
- 현재 3개 샘플을 넘어 golden 데이터셋 확장
- parity 및 leakage 테스트를 계속 green 상태로 유지

### 단기

- `v0.2-fsd50k-extended`가 희소 레이블 F1(`boxy`, `nasal`, `thin`)을 개선하면 프로덕션으로 승격
...
2026년 4월 15일 기준으로 `v0.1-real-data`에 대해 두 문장 모두 참입니다. 이 모델은 실제 공개 오디오로 학습되어 CI 평가를 통과했으며 현재 기본값입니다. 일부 레이블은 약하지만 더 이상 synthetic 데이터 전용이 아닙니다. 자세한 내용은 `ml/model_history.md`를 참고하세요.
